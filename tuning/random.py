...
class Random(TuningStrategy):
    ...
    def evaluate(self, batch_index, eval_env):
        # Evaluate every configuration
        rewards = []
        for j_config in range(self.num_configs):
            r = [self._run_evaluation(self.models[j_config], eval_env) for i in range(10)]
